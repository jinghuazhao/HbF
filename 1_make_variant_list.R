...
require(tidyr)
HbF <- "/rds/project/jmmh2/rds-jmmh2-projects/olink_proteomics/scallop/HbF"
hbf_xlsx <- file.path(HbF,"doc","hbf_GWAS_top_snps.xlsx")
if(!dir.exists(file.path(HbF,"work"))) dir.create(file.path(HbF,"work"))
hbf_hits <- read.xlsx(file.path(HbF,hbf_xlsx),sheet=1,colNames=TRUE,startRow=1,skipEmptyRows=TRUE) %>%
...
write.table(select(hbf_hits,-seqnames,-start,-end),file=file.path(HbF,"work","hbf_GWAS_top_snps.txt"),row.names=FALSE,quote=FALSE,sep="\t")
write.table(hbf_hits_long,file=file.path(HbF,"work","hbf_GWAS_top_snps_long.txt"),row.names=FALSE,quote=FALSE,sep="\t")
...
write.table(hbf_hits_region,file=file.path(HbF,"work","hbf_hits.txt"),row.names=FALSE,quote=FALSE,sep="\t")
